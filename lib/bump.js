...
const DEFAULT_OPTIONS = {
    ...
    /**
     * Set the new version via this setting instead of a prompt
     * 
    ...
    /**
     * Run the `rebuild` script if present in the updating packages scripts field
     * 
     * @type {boolean | undefined}
     */
    ...
    /**
     * Run the `test` script if defined in the `package.json` at the repositories root
     * 
     * @type {boolean | undefined}
     */
...
export default async function bump( REPOSITORY, opts = {} ) {
    ...
    // Stores all package version bumps made
    ...
    await ExecuteForEach( PACKAGES, NPM_VERSION_COMMAND, LogWorkspaceUpdate )
    ...
    // Run any rebuild script in `package.json` files for the bumped packages
    if ( opts.rebuild ) await ExecuteForEach( PACKAGES, `npm run rebuild --if-present` )

    // Just to be sure, run the test script (must be defined in the repository's root `package.json`)
    if ( opts.test ) await x( `npm run test --if-present` )

    const $newVersions = Array.from( VERSIONS )
    if ( $newVersions.length === 1 ) {

        if ( $newVersions[ 0 ] !== NEW_VERSION ) NEW_VERSION = $newVersions[ 0 ]

    } else {

        NEW_VERSION = await clack.text( {
            message: "Multiple versions bumped; choose a tag name",
        } )
    ...
    if ( NEW_VERSION.startsWith( "v" ) ) NEW_VERSION = NEW_VERSION.slice( 1 )
    ...
    await x( `git commit -m ${ $newVersions.join( ", " ) }` )
    await x( `git tag v${ NEW_VERSION } -m ${ $newVersions.join( ", " ) }` )

    // We only need to publish packages not marked private, so find them first...
    const PUBLIC_PACKAGES = /** @type {string[]} */ ( [] )
    await serial( PACKAGES, async dir => {

        const meta = await readJSON( join( dir, "./package.json" ) )
        if ( meta.private === true ) {

            log.info( "bump: Skipping private package", color.cyan( meta.name ) )
            return;

        }

        PUBLIC_PACKAGES.push( dir )

    } )

    // Publish every public workspace package to NPM; ensure we stop on a error
    await ExecuteForEach( PUBLIC_PACKAGES, `npm publish --access public` )
